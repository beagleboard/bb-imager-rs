...
pub(crate) fn update(state: &mut BBImager, message: BBImagerMessage) -> Task<BBImagerMessage> {
    match message {
        ...
        BBImagerMessage::UiState(Message::Next) => match std::mem::take(state) {
            BBImager::Customize(mut inner) => {
                match &inner.inner.customization {
                    bb_imager_ui::customization::Customization::SysConfig(x)
                    | bb_imager_ui::customization::Customization::SelectableSd(
                    ...
                    bb_imager_ui::customization::Customization::CloudInit(x)
                    | bb_imager_ui::customization::Customization::SelectableSd(
                    ...
                    if let Some(customization) = helpers::no_customization(flasher, &img) {
                        ...
                        *state = BBImager::Review(ctx.review(page.common));
                        ...
                        let customization = match flasher {
                            ...
                            {
                                bb_imager_ui::customization::Customization::SysConfig(
                                    (&page.common.app_config).into(),
                                )
                            }
                            bb_config::config::Flasher::SdCard
                                if img.init_format()
                            ...
                            {
                                bb_imager_ui::customization::Customization::CloudInit(
                                    (&page.common.app_config).into(),
                                )
                            }
                            bb_config::config::Flasher::SdCard => {
        ...
        // Nothing on the app-options page is editable yet, so there is nothing to save.
        BBImagerMessage::UiState(Message::Save) => {}
        BBImagerMessage::UiState(Message::Null) => {}
        ...
        BBImagerMessage::Null => {}
        _ => {
            tracing::error!("Unimplemented message: {:#?}", message);
            unimplemented!()
        }
    }
