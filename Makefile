...
APPIMAGE_ARCH ?= $(_ARCH)
## variable: APPIMAGE_SIGN_KEY: GPG key to use for signing.
APPIMAGE_SIGN ?= 0
...
ifeq ($(APPIMAGE_SIGN),1)
	_APPIMAGETOOL_ARGS += --sign
endif
...
package-gui-appimage: build-gui
	...
	appimagetool $(_APPIMAGETOOL_ARGS) bb-imager-gui/dist/org.beagleboard.imagingutility.AppDir bb-imager-gui/dist/BeagleBoard_Imager-$(VERSION)-$(APPIMAGE_ARCH).AppImage
