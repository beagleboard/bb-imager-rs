...
fn flash_sd_without_customization_flags_writes_no_config() {
    ...
    assert!(
        mock.boot_file("sysconf.txt").is_err(),
...
fn flash_sd_without_usb_dhcp_flag_omits_the_key() {
    ...
    assert_eq!(mock.boot_file("sysconf.txt").unwrap(), "keymap=us\n");
}

/// Wi-Fi credentials are split across two files: the sysconfig key points at a
/// per-SSID PSK file written under `services/`. The image here has no
/// `services/` directory, so this also covers the customization writer creating
/// the missing parent.
#[test]
...
fn flash_sd_cloud_init_emits_both_configs() {
    ...
    let cloud_init = mock.boot_file("cloud-init").unwrap();
    assert!(
    ...
    assert!(
        mock.boot_file("sysconf.txt").is_ok(),
